  ...
  const handleGenerateNew = async ()=>{
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.response?.data?.error || 'An error occurred while generating a new prediction.',
      });
    }finally{
      setLoading(false)
    }
                  ...
                  {prediction.skillGaps?.map((skill: any, index: number) => (
                    <div key={index} className="border border-gray-200 rounded-xl p-4 bg-white">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${skill.priority === 'Very High' ? 'bg-red-100 text-red-800' :
                    ...
                    <div key={index} className="border border-gray-200 rounded-xl p-6">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <span className={`px-3 py-1 rounded-full text-sm font-semibold ${target.readiness >= 80 ? 'bg-green-100 text-green-800' :
          ...
          {activeTab === 'learning' && (
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <div className={`p-4 rounded-xl ${motivationAnalysis.burnoutRisk === 'Low' ? 'bg-green-50' :
                        motivationAnalysis.burnoutRisk === 'Medium' ? 'bg-yellow-50' : 'bg-red-50'
                      }`}>
                      <div className="text-sm text-gray-600">Burnout Risk</div>
                      <div className={`text-lg font-bold ${motivationAnalysis.burnoutRisk === 'Low' ? 'text-green-700' :
                      ...
                      {Object.entries(interviewReadiness.technical || {}).map(([skill, data]: [string, any]) => (
                        <div key={skill} className="p-4 bg-gray-50 rounded-xl">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${data.priority === 'Very High' ? 'bg-red-100 text-red-800' :
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className={`text-sm font-medium ${data.status === 'Critical Gap' ? 'text-red-600' :
                    ...
                    <div className="space-y-4">
                      {Object.entries(interviewReadiness.behavioral || {}).map(([skill, data]: [string, any]) => (
                        <div key={skill} className="p-4 bg-gray-50 rounded-xl">
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-semibold text-gray-900 capitalize">{skill}</span>
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${data.priority === 'High' ? 'bg-orange-100 text-orange-800' :
                                'bg-yellow-100 text-yellow-800'
                              }`}>
                              {data.priority}
                            </span>
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-2xl font-bold text-blue-600">{data.score}%</span>
                            <span className={`text-sm font-medium ${data.status === 'Good' ? 'text-green-600' : 'text-yellow-600'
                              }`}>
                              {data.status}
                            </span>
                          </div>
